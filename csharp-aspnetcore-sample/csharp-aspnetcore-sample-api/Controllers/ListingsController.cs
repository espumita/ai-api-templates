...
public class ListingsController : ControllerBase {
    ...
    public async Task<ActionResult<PaginatedListingsResponse>> GetListings(
        ...
        IEnumerable<Listing> listings = await _listingRepository.GetAllAsync(page, pageSize, latitude, longitude);
        int totalItems = await _listingRepository.GetTotalCountAsync();

        var response = new PaginatedListingsResponse {
            Items = listings, TotalItems = totalItems, Page = page, PageSize = pageSize
        };
    ...
    public async Task<ActionResult<Listing>> CreateListing(Listing listing) {
        ...
        Listing? createdListing = await _listingRepository.CreateAsync(listing);
    ...
    public async Task<ActionResult<Listing>> UpdateListing(Guid id, Listing listing) {
        ...
        bool exists = await _listingRepository.ExistsAsync(id);
        if (!exists) {
            return NotFound();
        }

        Listing? updatedListing = await _listingRepository.UpdateAsync(listing);
    ...
    public async Task<IActionResult> DeleteListing(Guid id) {
        bool deleted = await _listingRepository.DeleteAsync(id);
        if (!deleted) {
            return NotFound();
        }
    ...
    public async Task<ActionResult<SearchResponse>> SearchListings([FromBody] FilterRequest filterRequest) {
        ...
        // Validate latitude and longitude if provided
        if (filterRequest.Latitude.HasValue &&
            (filterRequest.Latitude.Value < -90 || filterRequest.Latitude.Value > 90)) {
            return BadRequest("Latitude must be between -90 and 90 degrees");
        }

        if (filterRequest.Longitude.HasValue &&
            (filterRequest.Longitude.Value < -180 || filterRequest.Longitude.Value > 180)) {
            return BadRequest("Longitude must be between -180 and 180 degrees");
        }
        ...
        string[] validFields = new[] { "name", "description", "category", "location.country", "location.municipality" };
        string[] validOperators = new[] { "contains", "equals" };

        foreach (Filter filter in filterRequest.Filters) {
            if (!validFields.Contains(filter.Field.ToLowerInvariant())) {
                return BadRequest(
                    $"Invalid filter field: {filter.Field}. Valid fields are: {string.Join(", ", validFields)}");
            }

            if (!validOperators.Contains(filter.Operator.ToLowerInvariant())) {
                return BadRequest(
                    $"Invalid filter operator: {filter.Operator}. Valid operators are: {string.Join(", ", validOperators)}");
            }
        ...
        (IEnumerable<Listing> items, int totalCount) = await _listingRepository.SearchAsync(filterRequest.Filters,
            filterRequest.Page, filterRequest.PageSize, filterRequest.Latitude, filterRequest.Longitude);
